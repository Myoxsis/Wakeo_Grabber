...
const setQueuePauseUi = () => {
  queuePauseButton.disabled = !queueRunning;
...
const processQueue = async () => {
  ...
  setQueuePauseUi();
  renderHistory(data.capturedLinks || []);
    ...
    for (let index = 0; index < captures.length; index += 1) {
      ...
      await new Promise((resolve) => {
        chrome.tabs.create({ url: capture.url, active: false }, async (tab) => {
          ...
          await delayWithPause(getRandomQueueDwellWait());
      ...
      if (index < captures.length - 1) {
        ...
        await delayWithPause(getRandomQueueBetweenWait());
      }
    }
  } finally {
    await resetQueueUi();
